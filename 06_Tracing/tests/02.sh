...
# Usage: ./tests/02.sh <move binary path>
...
OUTPUT_STRING="$BINARY: fstat() failed on file $INPUT_FILE: $ERR_DESCRIPTION"
EXITED_STRING="+++ exited with $ERR_CODE +++"

REFERENCE=`mktemp`
printf '%s\n%s\n' "$OUTPUT_STRING" "$EXITED_STRING" > $REFERENCE

strace -e fault=fstat:error=$ERR_NAME -P $INPUT_FILE $BINARY $INPUT_FILE $OUTPUT_FILE 2>&1 | grep -E "\+\+\+|$BINARY:" | cmp $REFERENCE || exit 1
